...
if (isset($vbulletin->options['hv_type'])) // vBulletin 3.7?
	{
	// get new threads
	$threads = $vbulletin->db->query_read_slave("
		SELECT
		user.userid, user.salt, user.username, user.email, user.languageid, user.usergroupid, user.membergroupids, user.jabber,
...
// get new threads
	...
	while ($thread = $vbulletin->db->fetch_array($threads))
	{
		...
		if ($haveothers)
		{
			...
			if($thread['jabber']) {
    if($vbulletin->options['jabber_selectclass'] == 1) $jabber_class->message($thread['jabber'], $message, "chat", $subject); // xmpphp
    else $jabber_class->send_message($thread['jabber'], $message, $subject); // legacy
    }
	}
	}
	...
	$forums = $vbulletin->db->query_read_slave("
		SELECT user.userid, user.salt, user.username, user.email, user.jabber, user.languageid, user.usergroupid, user.membergroupids,
			user.timezoneoffset, IF(user.options & " . $vbulletin->bf_misc_useroptions['dstonoff'] . ", 1, 0) AS dstonoff,
			IF(user.options & " . $vbulletin->bf_misc_useroptions['hasaccessmask'] . ", 1, 0) AS hasaccessmask,
...
if (isset($vbulletin->options['hv_type'])) // vBulletin 3.7?
{
	$threads = $vbulletin->db->query_read_slave("
		...
		if (!empty($newthreads) OR !empty($updatedthreadbits))
		{
			...
			if($forum['jabber']) {
    if($vbulletin->options['jabber_selectclass'] == 1) $jabber_class->message($forum['jabber'], $message, "chat", $subject); // xmpphp
    else $jabber_class->send_message($forum['jabber'], $message, $subject); // legacy
    }
		}
	}
