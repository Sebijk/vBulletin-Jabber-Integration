	...
	while ($thread = $vbulletin->db->fetch_array($threads))
	{
		...
		if ($haveothers)
		{
           ...
           $subject = utf8_encode($subject);
			...
			if($thread['jabber']) $jabber_class->message($thread['jabber'], $message, "chat", $subject);
    //Jabber End
		}
	}
	...
	while ($forum = $vbulletin->db->fetch_array($forums))
	{
      ...
      if($vbulletin->options['jabber_selectcharset'] == 1) {
           ...
           $subject = utf8_encode($subject);
			...
			if($forum['jabber']) $jabber_class->message($forum['jabber'], $message, "chat", $subject);
		}
	}
	...
	if ($vbulletin->options['socnet'] & $vbulletin->bf_misc_socnet['enable_groups'])
	{
        ...
        if($vbulletin->options['jabber_selectcharset'] == 1) {
           ...
           $subject = utf8_encode($subject);
				...
				if($group['jabber']) $jabber_class->message($group['jabber'], $message, "chat", $subject);
			}
		}
	}
